exports.request = function(req, res, mysqlclient) {
	this.MAD_INTERSTITIALS_EXACTMATCH = false;
	...
	this.check_input = function(data) {
		...
		if (typeof (this.request_settings['ip_address']) == 'undefined'
				|| !is_valid_ip(this.request_settings['ip_address'])) {
		...
		if (typeof (this.request_settings['user_agent']) == 'undefined'
				|| this.request_settings['user_agent'] == '') {
	...
	this.check_forwarded_ip = function(data) {
		...
		if (isset(this.req.header('x-forwarded-for'))
				&& this.req.header('x-forwarded-for') != '') {
		 ...
		 * !empty($_SERVER['HTTP_X_FORWARDED_FOR'])){ $res_array = explode(",",
		 * $_SERVER['HTTP_X_FORWARDED_FOR']); return $res_array[0]; }
		 */
		return false;
	}
	this.prepare_ua = function(data) {
		// console.log( this.req.header('User-Agent'));
		if (typeof (this.req.header('User-Agent')) != "undefined"
				&& this.req.header('User-Agent') != '') {
	...
	this.getchannel = function(callback) {
		...
		if (this.zone_detail.zone_channel != '') {
			...
			this.get_publication_channel(this,
					this.zone_detail['publication_id'], callback);

		}

	}
	this.get_publication_channel = function(request, publication_id, callback) {
		var query = "SELECT inv_defaultchannel,creator_id FROM md_publications WHERE inv_id='"
				+ publication_id + "'";
	...
	this.update_last_request = function() {
		...
		if (lastreq_dif >= 600 || this.zone_detail.zone_lastrequest < 1) {
			this.client.query("UPDATE md_zones set zone_lastrequest='"
					+ this.timestamp + "' where entry_id='"
					+ this.zone_detail.entry_id + "'");
			this.client.query("UPDATE md_publications set md_lastrequest='"
					+ this.timestamp + "' where inv_id='"
					+ this.zone_detail.publication_id + "'");
			// global.client.query($query,function selectCb(err, results,
			// fields) {
	...
	this.set_device = function() {
		...
		if (detect.iPhone) {
			...
			this.print_error(1,
					'This ad-server does not serve ads to non-mobile devices.',
					this.request_settings['sdk'], 1);
			return false;
		}
		// console.log(temp);
		if (typeof (temp['device_os']) != 'undefined'
				&& temp['device_os'] != '') {
	...
	this.check_cron_active = function(request, callback) {
		...
		this.client.query(query, function selectCb(err, results, fields) {
			...
			if (!results || results.length == 0) {
				...
				if (results[0].var_value == null || results[0].var_value == ''
						|| results[0].var_value <= 0) {
					request.request_settings['cron_active'] = false;
				} else {
	...
	this.build_query = function() {
		...
		var zone_detail = this.zone_detail;
		var query_part = [];
		if (typeof (request_settings['geo_country']) != 'undefined'
				&& request_settings['geo_country'] != ''
				&& typeof (request_settings['geo_region']) != 'undefined'
				&& request_settings['geo_region'] != '') {
			query_part['geo'] = " OR (c1.targeting_type='geo' AND (c1.targeting_code='"
					+ request_settings['geo_country']
					+ "' OR c1.targeting_code='"
					+ request_settings['geo_country']
					+ "_"
					+ request_settings['geo_region'] + "')))";
		} else if (typeof (request_settings['geo_country']) != 'undefined'
				&& request_settings['geo_country'] != '') {
			query_part['geo'] = " OR (c1.targeting_type='geo' AND c1.targeting_code='"
					+ request_settings['geo_country'] + "'))";
		} else {
		...
		if (typeof (request_settings['channel']) != 'undefined'
				&& (request_settings['channel'])) {
			query_part['channel'] = "AND (md_campaigns.channel_target=1 OR (c2.targeting_type='channel' AND c2.targeting_code='"
					+ request_settings['channel'] + "'))";
		} else {
			query_part['channel'] = '';
		}

		query_part['placement'] = "AND (md_campaigns.publication_target=1 OR (c3.targeting_type='placement' AND c3.targeting_code='"
				+ zone_detail.entry_id + "'))";

		query_part['misc'] = "AND md_campaigns.campaign_status=1 AND md_campaigns.campaign_start<='"
				+ date('Y-m-d')
				+ "' AND md_campaigns.campaign_end>'"
				+ date('Y-m-d') + "'";
		...
		if (request_settings['main_device'] != 'OTHER'
				&& request_settings['main_device'] != 'NOMOBILE') {
			switch (request_settings['main_device']) {

			case 'IPHONE':
			case 'IPOD':
			case 'IPAD':
				if (typeof (request_settings['device_os']) != 'undefined'
						&& (request_settings['device_os'])) {
					query_part['osversion'] = "AND ((md_campaigns.ios_version_min<='"
							+ request_settings['device_os']
							+ "' OR md_campaigns.ios_version_min='') AND (md_campaigns.ios_version_max>='"
							+ request_settings['device_os']
							+ "' OR md_campaigns.ios_version_max=''))";
				} else {
			...
			case 'ANDROID':
				if (typeof (request_settings['device_os']) != 'undefined'
						&& (request_settings['device_os'])) {
					query_part['osversion'] = "AND ((md_campaigns.android_version_min<='"
							+ request_settings['device_os']
							+ "' OR md_campaigns.android_version_min='') AND (md_campaigns.android_version_max>='"
							+ request_settings['device_os']
							+ "' OR md_campaigns.android_version_max=''))";
				} else {
		...
		if (request_settings['latitude'] != ''
				&& request_settings['longitude'] != '') {
			var lng = request_settings['longitude'];
			var lat = request_settings['latitude'];
			query_part['map'] = '(md_campaigns.map_target=1 or (map.left_top_lng<='
					+ lng
					+ ' and '
					+ lng
					+ '<=map.right_bottom_lng and '
					+ lat
					+ '<= map.left_top_lat and map.right_bottom_lat<='
					+ lat
					+ '))';
		} else {
			query_part['map'] = '1';
		 ...
		 * md_campaigns.campaign_networkid from md_campaigns LEFT JOIN
		 * md_campaign_targeting c1 ON md_campaigns.campaign_id = c1.campaign_id
		 * LEFT JOIN md_campaign_targeting c2 ON md_campaigns.campaign_id =
		 * c2.campaign_id LEFT JOIN md_campaign_targeting c3 ON
		 * md_campaigns.campaign_id = c3.campaign_id LEFT JOIN md_ad_units ON
		 * md_campaigns.campaign_id = md_ad_units.campaign_id LEFT JOIN
		 * md_campaign_limit ON md_campaigns.campaign_id =
		 * md_campaign_limit.campaign_id where
		 * (md_campaigns.country_target=1".$query_part['geo']."
		 * ".$query_part['channel']." ".$query_part['placement']."
		 * ".$query_part['misc']." ".$query_part['device']."
	...
	this.launch_campaign_query = function(request, callback) {
		...
		this.client
				.query(
						this.request_settings['campaign_query'],
						function selectCb(err, results, fields) {
							if (err) {
								throw err;
							}
							...
							if (!results || results.length == 0) {
								 ...
								 * for(var i in campaignarray){ campaign_id[i] =
								 * campaignarray[i]['campaign_id']; priority[i] =
								 * campaignarray[i]['priority']; type[i] =
								 * campaignarray[i]['type']; network_id[i] =
								 * campaignarray[i]['network_id']; }
								 */
								campaignarray.sort(function() {
									return Math.random() > 0.5 ? -1 : 1;
								});
								campaignarray.sort(function(x, y) {
	...
	this.build_ad = function($type, $content) {
		...
		if ($type == 1) {
			...
			case 'banner':
				...
				if (this.MAD_CLICK_ALWAYS_EXTERNAL
						|| $content['adv_click_opentype'] == 'external') {
				...
				case 3:
					$display_ad['html_markup'] = htmlspecialchars_decode($content['adv_chtml']);
			...
			case 'interstitial':
				...
				$display_ad['interstitial-orientation'] = 'portrait';// landscape
				$display_ad['interstitial-preload'] = 1;
				$display_ad['interstitial-autoclose'] = 0;
				$display_ad['interstitial-type'] = 'url';// 'markup'
				$display_ad['interstitial-skipbutton-show'] = 1;
				$display_ad['interstitial-skipbutton-showafter'] = 0;
				$display_ad['interstitial-navigation-show'] = 1;
				$display_ad['interstitial-navigation-topbar-show'] = 1;
				$display_ad['interstitial-navigation-bottombar-show'] = 1;
				$display_ad['interstitial-navigation-topbar-custombg'] = '';
				$display_ad['interstitial-navigation-bottombar-custombg'] = '';
				$display_ad['interstitial-navigation-topbar-titletype'] = 'variable';// variable,fixed:
				$display_ad['interstitial-navigation-topbar-titlecontent'] = '';
				$display_ad['interstitial-navigation-bottombar-backbutton'] = 1;
				$display_ad['interstitial-navigation-bottombar-forwardbutton'] = 1;
				$display_ad['interstitial-navigation-bottombar-reloadbutton'] = 1;
				$display_ad['interstitial-navigation-bottombar-externalbutton'] = 1;
				$display_ad['interstitial-navigation-bottombar-timer'] = 1;
				...
				switch ($content['adv_type']) {
				case 1:
					$display_ad['interstitial-content'] = '<meta content="width=device-width; initial-scale=1.0; maximum-scale=1.0; user-scalable=0;" name="viewport" /><meta name="viewport" content="width=device-width" /><div style="position:absolute;top:0;left:0;"><a href="mfox:external:'
							+ $content['adv_click_url']
							+ '"><img src="'
							+ this.get_creative_url($content)
							+ '"></a>'
				...
				case 2:
					$display_ad['interstitial-content'] = '<meta content="width=device-width; initial-scale=1.0; maximum-scale=1.0; user-scalable=0;" name="viewport" /><meta name="viewport" content="width=device-width" /><div style="position:absolute;top:0;left:0;"><a href="mfox:external:'
							+ $content['adv_click_url']
							+ '"><img src="'
							+ $content['adv_bannerurl']
							+ '"></a>'
							+ $tracking_pixel_html + '</div>';
					break;

				case 3:
					$display_ad['interstitial-content'] = htmlspecialchars_decode($content['adv_chtml'])
							+ $tracking_pixel_html;
					break;
			...
			case 'display':
				...
				case 'markup':
					$valid_ad = 1;
					if (typeof ($display_ad['html_markup'] == 'undefined')
							|| empty($display_ad['html_markup'])) {
						global.display_ad = $display_ad;
						return false;
					}
					if (!isset($display_ad['click_url'])
							|| empty($display_ad['click_url'])) {
						if (!$display_ad['click_url'] != extract_url($display_ad['html_markup'])) {
							this.display_ad = $display_ad;
							return false;
						}
					}
					if (!isset($display_ad['clicktype'])
							|| empty($display_ad['clicktype'])) {
						$display_ad['clicktype'] = 'safari';
					}
					if (!isset($display_ad['refresh'])
							|| empty($display_ad['refresh'])) {
						$display_ad['refresh'] = $zone_detail['zone_refresh'];
					}
					if (!isset($display_ad['skipoverlay'])
							|| empty($display_ad['skipoverlay'])) {
						$display_ad['skipoverlay'] = 0;
					}
					if (!isset($display_ad['skippreflight'])
							|| empty($display_ad['skippreflight'])) {
						$display_ad['skippreflight'] = 'yes';
					}
					break;
				...
				case 'mraid-markup':
					$valid_ad = 1;
					if (!isset($display_ad['html_markup'])
							|| empty($display_ad['html_markup'])) {
						return false;
					}
					if (!isset($display_ad['clicktype'])
				...
				case 'image-url':
					...
					if (!isset($display_ad['image_url'])
							|| empty($display_ad['image_url'])) {
						return false;
					}
					if (!isset($display_ad['clicktype'])
							|| empty($display_ad['clicktype'])) {
			...
			case 'interstitial':
				...
				/*
				 * switch ($display_ad['type']){ We might add some validation
				 * for Interstitials later. }
				 */
	...
	this.get_creative_url = function(content) {
		var imageurl = this.SITE_URL + '/data/creative/' + content.unit_hash
				+ '.' + content.adv_creative_extension;
		return imageurl;
	}
	...
	this.track_request = function($impression) {
		...
		case 'normal':
			this.reporting_db_update($zone_detail['publication_id'],
					$zone_detail['entry_id'], $display_ad['campaign_id'],
					$display_ad['ad_id'], '', 1, 0, $impression, 0);
			break;

		case 'network':
			this.reporting_db_update($zone_detail['publication_id'],
					$zone_detail['entry_id'],
					$request_settings['active_campaign'], '',
					$request_settings['network_id'], 1, 0, $impression, 0);
			break;
		...
		default:
			...
			this.reporting_db_update($zone_detail['publication_id'],
					$zone_detail['entry_id'], '', '', '', 1, 0, $impression, 0);
			break;
		}
	...
	this.reporting_db_update = function($publication_id, $zone_id,
			$campaign_id, $creative_id, $network_id, $add_request,
			$add_request_sec, $add_impression, $add_click) {
		...
		var request = this;
		var $select_query = "select entry_id from md_reporting where publication_id='"
				+ $publication_id
				+ "' AND zone_id='"
				+ $zone_id
				+ "' AND campaign_id='"
						...
						function selectCb(err, results, fields) {
							...
							if (!results || results.length == 0) {
								...
								var $sql = "UPDATE md_reporting set total_requests=total_requests+"
										+ $add_request
										+ ", total_requests_sec=total_requests_sec+"
										+ $add_request_sec
	...
	this.deduct_impression = function($campaign_id) {
		var $query = "UPDATE md_campaign_limit set total_amount_left=total_amount_left-1 WHERE campaign_id='"
				+ $campaign_id + "' AND total_amount>0 and total_amount_left>0";
	...
	this.display_ad_ = function() {
		...
		// echo "Displaying ad..."; global $display_ad; print_r($display_ad);
		// global $request_settings; print_r($request_settings); global
		// $zone_detail; print_r($zone_detail); exit;
		var d = new Date();
		var t = d.getMinutes().toString() + ':' + d.getSeconds().toString()
	...
	this.prepare_ctr = function() {
		...
		var $request_settings = this.request_settings;
		var $zone_detail = this.zone_detail;
		// $base_ctr="".MAD_ADSERVING_PROTOCOL . $_SERVER['HTTP_HOST'] .
		// dirname($_SERVER['PHP_SELF'])."/".MAD_CLICK_HANDLER."?zone_id=".$zone_detail['entry_id']."&h=".$request_settings['request_hash']."";
		// var
		// $base_ctr=this.SERVER_URL+"/click?zone_id="+$zone_detail['entry_id']+"&h="+$request_settings['request_hash'];
		var $base_ctr = SITE_URL + '/admin/show/click/id/'
				+ $display_ad['ad_id'] + '/zone_id/' + $zone_detail['entry_id']
				+ '/campaign_id/' + $request_settings['final_ad'].campaign_id
		...
		if ($display_ad['main_type'] == 'display') {
			...
			case 'image-url':
				if ($request_settings['response_type'] != 'xml') {
					var $final_markup = '<a id="mAdserveAdLink" href="'
			...
			if (isset($display_ad['trackingpixel'])
					&& !empty($display_ad['trackingpixel'])
					&& $display_ad['trackingpixel'] != '') {
				$final_markup = $final_markup
						+ generate_trackingpixel($display_ad['trackingpixel']);
			}
	...
	this.generate_final_markup = function() {
		...
		if (isset($display_ad['click_url']) && !empty($display_ad['click_url'])) {
			$markup = str_replace($display_ad['click_url'],
					$display_ad['final_click_url'], $display_ad['html_markup']);
		} else {
	...
	this.print_ad = function() {
		...
		if ($display_ad['main_type'] == 'display') {
			...
			case 'xml':
				...
				str += "<htmlString skipoverlaybutton=\""
						+ $display_ad['skipoverlay'] + "\"><![CDATA[";
			...
			case 'json':
				 ...
				 */
				str += 'var ' + jsvar + ' = [{"url" : "'
						+ $display_ad['final_click_url'] + '","content" : "'
						+ addslashes($display_ad['final_markup'])
						+ '", "track" : ""}];';
				break;
			...
			str += '<ad type="'
					+ convert_interstitial_name($display_ad['type'])
					+ '" animation="' + $display_ad['animation'] + '">';

			if ($display_ad['type'] == 'interstitial'
					|| $display_ad['type'] == 'video-interstitial'
					|| $display_ad['type'] == 'interstitial-video') {
				if ($display_ad['interstitial-type'] == 'markup') {
					var $interstitial_urlcontent = '';
				} else {
					var $interstitial_urlcontent = 'url="' + SITE_URL
							+ '/admin/show/getcontent/id/'
							+ $display_ad['ad_id'] + '/zone_id/'
							+ $zone_detail['entry_id'] + '/campaign_id/'
							+ $request_settings['final_ad'].campaign_id + '/h/'
							+ $request_settings['request_hash'] + '"';
				}
				str += '<interstitial preload="'
						+ $display_ad['interstitial-preload'] + '" autoclose="'
						+ $display_ad['interstitial-autoclose'] + '" type="'
						+ $display_ad['interstitial-type'] + '" '
						+ $interstitial_urlcontent + ' orientation="'
						+ $display_ad['interstitial-orientation'] + '">';
				if ($display_ad['interstitial-type'] == 'markup') {
					str += '<markup><![CDATA['
							+ $display_ad['interstitial-content']
							+ ']]></markup>';
				}
				str += '<skipbutton show="'
						+ $display_ad['interstitial-skipbutton-show']
						+ '" showafter="'
						+ $display_ad['interstitial-skipbutton-showafter']
						+ '"></skipbutton>';
				str += '<navigation show="'
						+ $display_ad['interstitial-navigation-show'] + '">';
				str += '<topbar custombackgroundurl="'
						+ $display_ad['interstitial-navigation-topbar-custombg']
						+ '" show="'
						+ $display_ad['interstitial-navigation-topbar-show']
						+ '" title="'
						+ $display_ad['interstitial-navigation-topbar-titletype']
						+ '" titlecontent="'
						+ $display_ad['interstitial-navigation-topbar-titlecontent']
						+ '"></topbar>';
				str += '<bottombar custombackgroundurl="'
						+ $display_ad['interstitial-navigation-bottombar-custombg']
						+ '" show="'
						+ $display_ad['interstitial-navigation-bottombar-show']
						+ '" backbutton="'
						+ $display_ad['interstitial-navigation-bottombar-backbutton']
						+ '" forwardbutton="'
						+ $display_ad['interstitial-navigation-bottombar-forwardbutton']
						+ '" reloadbutton="'
						+ $display_ad['interstitial-navigation-bottombar-reloadbutton']
						+ '" externalbutton="'
						+ $display_ad['interstitial-navigation-bottombar-externalbutton']
						+ '" timer="'
						+ $display_ad['interstitial-navigation-bottombar-timer']
						+ '">';
				str += '</bottombar>';
				str += '</navigation>';
				str += '</interstitial>';
			...
			if ($display_ad['type'] == 'video'
					|| $display_ad['type'] == 'video-interstitial'
					|| $display_ad['type'] == 'interstitial-video') {

				str += '<video orientation="'
						+ $display_ad['video-orientation'] + '" expiration="'
						+ $display_ad['video-expiration'] + '">';
				str += '<creative display="'
						+ $display_ad['video-creative-display']
						+ '" delivery="'
						+ $display_ad['video-creative-delivery'] + '" type="'
						+ $display_ad['video-creative-type'] + '" bitrate='
						+ $display_ad['video-creative-bitrate'] + '"" width="'
						+ $display_ad['video-creative-width'] + '" height="'
						+ $display_ad['video-creative-height'] + '"><![CDATA['
						+ $display_ad['video-creative-url'] + ']]></creative>';
				str += '<duration>' + $display_ad['video-duration']
						+ '</duration>';
				str += '<skipbutton show="'
						+ $display_ad['video-skipbutton-show']
						+ '" showafter="'
						+ $display_ad['video-skipbutton-showafter']
						+ '"></skipbutton>';
				str += '<navigation show="'
						+ $display_ad['video-navigation-show'] + '" allowtap="'
						+ $display_ad['video-navigation-allowtap'] + '">';
				str += '<topbar custombackgroundurl="'
						+ $display_ad['video-navigation-topbar-custombg']
						+ '" show="'
						+ $display_ad['video-navigation-topbar-show']
						+ '"></topbar>';
				str += '<bottombar custombackgroundurl="'
						+ $display_ad['video-navigation-bottombar-custombg']
						+ '" show="'
						+ $display_ad['video-navigation-bottombar-show']
						+ '" pausebutton="'
						+ $display_ad['video-navigation-bottombar-pausebutton']
						+ '" replaybutton="'
						+ $display_ad['video-navigation-bottombar-replaybutton']
						+ '" timer="'
						+ $display_ad['video-navigation-bottombar-timer']
						+ '">';
				str += '</bottombar>';
				str += '</navigation>';
				str += '<trackingevents>';
				for ( var i in $display_ad['video-trackers']) {
					str += '<tracker type="'
							+ $display_ad['video-trackers'][i][0]
							+ '"><![CDATA[' + $display_ad['video-trackers'][i]
							+ ']]></tracker>';
				}
				...
				if ($display_ad['video-htmloverlay-show'] == 1) {
					if ($display_ad['video-htmloverlay-type'] == 'markup') {
						var $htmloverlay_urlcontent = '';
					} else {
						var $htmloverlay_urlcontent = 'url="'
								+ htmlspecialchars($display_ad['video-htmloverlay-content'])
								+ '"';
					}
					str += '<htmloverlay show="'
							+ $display_ad['video-htmloverlay-show']
							+ '" showafter="'
							+ $display_ad['video-htmloverlay-showafter']
							+ '" type="'
							+ $display_ad['video-htmloverlay-type'] + '" '
							+ $htmloverlay_urlcontent + '>';
					if ($display_ad['video-htmloverlay-type'] == 'markup') {
						str += '<![CDATA['
								+ $display_ad['video-htmloverlay-content']
								+ ']]>';
					}
	...
	this.exchangebalance = function() {
		...
		if (this.request_settings['final_ad']['cost_type'] == 1) {
			if (this.request_settings['final_ad']['money'] > 0) {
				...
				//console.log(e);
				var query = "update md_campaign_limit set money=money-" + e
						+ " where  campaign_id=" + campaign_id + " and money>0";
				this.client.query(query);
				query = "update md_zones set income=income+" + e
						+ " where entry_id=" + zone_id;
				this.client.query(query);
				query = "update md_publications set income=income+" + e
						+ " where inv_id=" + publication_id;
				this.client.query(query);
				query = "update md_reporting set money=money+" + e
						+ " where  publication_id='" + publication_id
						+ "' AND zone_id='" + zone_id + "' AND campaign_id='"
						+ campaign_id + "' AND creative_id='" + creative_id
						+ "' AND date='" + date('Y-m-d') + "'";
				this.client.query(query);
				query = "update md_uaccounts set balance=balance+" + e
						+ " where user_id=" + this.zone_detail['creator_id'];
				this.client.query(query);
			}
